...
# TODO DONE: Implement Show and Artist models, and complete all model relationships and proprties, as a database migration.
...
Show = db.Table('Show', db.Model.metadata,
                db.Column('Venue_id', db.Integer, db.ForeignKey('Venue_id')),
...
class Venue(db.Model):
    ...
    seeking_description = db.Column(db.String(500)) #Because descriptions can be a little bit longer, I decided to accept input uo to 500 characters.
    venues = db.relationship('Artist', secondary=Show, backref=db.backref('shows', lazy='joined'))
    def __repr__(self):
        return 'Veneu Id:{} | Name:{}'.format(self.id, self.name)
...
class Artist(db.Model):
    ...
    genres = db.Column(db.ARRAY(db.String())) # To store multiple Genres, I decided to create an Array Column with String as Datatype
    seeking_description = db.Column(db.String(500)) # Because description can be a little bit longer, I decided to accept input up to 500 characters
